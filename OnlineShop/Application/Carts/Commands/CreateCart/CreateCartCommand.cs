        ...
        public async Task<string> Handle(CreateCartCommand request, CancellationToken cancellationToken)
        {
            ...
            if (productAsset.StoreId == indexAsset.FirstOrDefault().StoreId && indexAsset.Count() > 1)
            {
                ...
                if (_context.Carts.Any(x => x.ProductId == request.ProductId))
                {
                    ...
                    //Index Total Cart Price Update
                    var c = indexAsset.FirstOrDefault().TotalCartPrice;
                    indexAsset.FirstOrDefault().TotalCartPrice = c + cartEntity.TotalPrice;
                ...
                //Calculate total cart price in CartIndex Table
                var b = indexAsset.FirstOrDefault().TotalCartPrice;
                indexAsset.FirstOrDefault().TotalCartPrice = b + cartEntity.TotalPrice;
            ...
            //Calculate Total Cart Price in CartIndex Table
            var a = cartIndexEntity.TotalCartPrice;
            cartIndexEntity.TotalCartPrice = a + entity.TotalPrice;
